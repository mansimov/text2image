"""Theano implementation of the original DRAW model"""
...
class ReccurentAttentionVAE():
	...
	def save_weights(self, path, c_ts, read_attent_params, write_attent_params):
		...
		dset = weights_f.create_dataset('train_subdata', self.train_data[((self.n_batches-1) * batch_size):(self.n_batches * batch_size)].shape.eval(), dtype='f')
		dset[:] = np.copy(self.train_data[((self.n_batches-1) * batch_size):(self.n_batches * batch_size)].eval())
